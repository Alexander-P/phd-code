...
baseexp = Experiment('frierson', overwrite_data=False)
...
obls = np.arange(30, 31, 30)

for obl in obls:
	exp = baseexp.derive('frierson-diffusion_hc10-obl{}'.format(obl))
	exp.clear_rundir()
	exp.namelist['astronomy_nml']['obliq'] = obl
	exp.screen_runmonth_prefix = 'frierson obl %.1f' % (obl)
	exp.runmonth(1, use_restart=False,num_cores=8, light=False)
	for i in range(2,33):
		exp.runmonth(i, num_cores=8, light=False)
