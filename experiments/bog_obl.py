...
baseexp = Experiment('bog', overwrite_data=False)
...
baseexp.namelist['two_stream_gray_rad_nml']['bog_a'] = 0.1627
baseexp.namelist['two_stream_gray_rad_nml']['bog_b'] = 1997.9
...
obls = np.arange(0, 45, 15)

for obl in obls:
	exp = baseexp.derive('bog_ruthparams_hc10-obl{}'.format(obl))
	exp.clear_rundir()
	exp.namelist['astronomy_nml']['obliq'] = obl
	exp.screen_runmonth_prefix = 'bog obl %.1f' % (obl)
